...
//!
//! The theme itself is not in this repo. The August 2026 split brought the
//! theme *crate* — `fts-themer` — across from the monorepo and left the
//! asset directory it reads behind: `fts-theme/FastTrackStudio/`, holding
//! `rtconfig.txt` and `.source-art`. Every test here opens that file, so
//! all five fail on a missing path rather than on a mismatch, and they
//! have failed since the split landed.
//!
...
//!
//! Tracked as session#13 (the same assets, missing from session's copy of
//! this crate) and session#84. Restoring the assets is what un-ignores
//! these; the `#[ignore]`s come straight back off, and they should fail
//! loudly if the assets arrive in a different shape.
