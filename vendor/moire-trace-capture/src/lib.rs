...
mod platform {
    ...
    fn validate_frame_pointer_chain(min_depth: usize) -> Result<(), String> {
        ...
        for _ in 0..MAX_FRAMES {
            ...
            // Stop gracefully on misaligned or non-monotonic frame pointers rather
            // than failing. Some system libraries (e.g. glibc's dlopen dispatch path)
            // are built without -fno-omit-frame-pointer, so the chain may become
            // invalid before reaching the top of the stack. We only require that the
            // first `min_depth` frames are valid — those come from our own code which
            // is always compiled with force-frame-pointers=yes.
