...
pub(crate) fn diff_tracks(old: &[Track], new: &[Track]) -> Vec<TrackDiff> {
    ...
    for (&guid, &old_track) in &old_map {
        match new_map.get(guid) {
            ...
            Some(&new_track) => {
                if let Some(diff) = diff_single_track(old_track, new_track) {
                    diffs.push(diff);
                }
            }
        }
    ...
    for i in 0..common {
        if let Some(diff) = diff_single_track(old_no_guid[i].1, new_no_guid[i].1) {
            diffs.push(diff);
        }
    }
...
fn diff_single_track(old: &Track, new: &Track) -> Option<TrackDiff> {
    ...
    // Sub-diffs
    let item_diffs = item::diff_items(&old.items, &new.items);
    let env_diffs = envelope::diff_envelopes(&old.envelopes, &new.envelopes);
