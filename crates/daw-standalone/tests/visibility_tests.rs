...
async fn tracks_default_to_visible() {
    ...
    let cx = test_ctx();
    let tracks = track_svc.get_tracks(&cx, current()).await;
...
async fn hide_track_in_tcp() {
    ...
    // Hide it
    track_svc
        .set_visible_in_tcp(&cx, current(), TrackRef::Guid(guid.clone()), false)
        .await;

    // Verify hidden
    let track = track_svc
        .get_track(&cx, current(), TrackRef::Guid(guid))
        .await
        .unwrap();
...
async fn hide_track_in_mixer() {
    ...
    track_svc
        .set_visible_in_mixer(&cx, current(), TrackRef::Guid(guid.clone()), false)
        .await;
...
async fn toggle_visibility_roundtrip() {
    ...
    // Start visible
    let track = track_svc
        .get_track(&cx, current(), track_ref.clone())
        .await
        .unwrap();
    assert!(track.visible_in_tcp);
    ...
    // Hide
    track_svc
        .set_visible_in_tcp(&cx, current(), track_ref.clone(), false)
        .await;
    let track = track_svc
        .get_track(&cx, current(), track_ref.clone())
        .await
    ...
    // Show again
    track_svc
        .set_visible_in_tcp(&cx, current(), track_ref.clone(), true)
        .await;
    let track = track_svc
        .get_track(&cx, current(), track_ref)
        .await
        .unwrap();
    assert!(track.visible_in_tcp);
}
...
async fn hide_multiple_tracks_independently() {
    ...
    let updated = track_svc.get_tracks(&cx, current()).await;
...
async fn show_all_restores_visibility() {
    ...
    for track in &tracks {
        track_svc
            .set_visible_in_tcp(&cx, current(), TrackRef::Guid(track.guid.clone()), false)
            .await;
    }

    // Verify all hidden
    let hidden = track_svc.get_tracks(&cx, current()).await;
    for track in &hidden {
        assert!(
            !track.visible_in_tcp,
    ...
    for track in &tracks {
        track_svc
            .set_visible_in_tcp(&cx, current(), TrackRef::Guid(track.guid.clone()), true)
            .await;
    }

    // Verify all visible again
    let shown = track_svc.get_tracks(&cx, current()).await;
    for track in &shown {
        assert!(
            track.visible_in_tcp,
...
async fn to_track_propagates_all_state() {
    ...
    // Set various states
    track_svc
        .set_muted(&cx, current(), track_ref.clone(), true)
        .await;
    track_svc
        .set_soloed(&cx, current(), track_ref.clone(), true)
        .await;
    track_svc
        .set_visible_in_tcp(&cx, current(), track_ref.clone(), false)
        .await;
    track_svc
        .set_volume(&cx, current(), track_ref.clone(), 0.5)
        .await;
...
async fn visibility_by_index() {
    ...
    track_svc
        .set_visible_in_tcp(&cx, current(), TrackRef::Index(2), false)
        .await;

    // Verify by index
    let track = track_svc
        .get_track(&cx, current(), TrackRef::Index(2))
        .await
        .unwrap();
    assert!(!track.visible_in_tcp);
    ...
    let track0 = track_svc
        .get_track(&cx, current(), TrackRef::Index(0))
        .await
        .unwrap();
