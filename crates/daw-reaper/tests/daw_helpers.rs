...
use daw_control::{FxHandle, Project, TrackHandle};
use daw_proto::fx::tree::FxNodeKind;
use eyre::Result;
use signal::resolve::ResolvedGraph;
use signal::{
    Block, DawParameterSnapshot, LiveParam, MorphEngine, block_to_snapshot, find_param_index,
    graph_state_chunks, graph_to_snapshot, live_params_into_block,
};
...
/// Capture the FX chain content from a track as bytes via `daw-control`.
pub async fn capture_rfxchain_bytes(track: &TrackHandle) -> Result<Vec<u8>> {
    let text = track.fx_chain().fx_chain_chunk_text().await?;
    Ok(text.into_bytes())
}
